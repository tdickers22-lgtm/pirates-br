...
|---|---|---|---|---|---|
| ~~`--report`~~ VOID | 96214750 | 1 (in-process) | ~~4~~ (lost matches, see below) | 6.8 | 2026-09-24 |
| ~~`--report --workers 2`~~ VOID | 96214750 | 2 workers + lobby | ~~1~~ (lost matches, see below) | 10.4 | 2026-09-24 |

Both rows above are VOID, and so is the first `--scaling 2` verdict (ratio 0.25). The harness lost the matches it claimed to measure: the fast-forward runs 300 sim seconds against a socket the blocked event loop cannot drain, `enforceCongestion` counts sim time, so each match evicted its own load crew during its fast-forward, the held seat expired 60 s later and the match was abandoned and reaped. The in-process "4" rows carried about 2 live matches, and under workers the previous match was evicted while the lobby thread sat in `Atomics.wait`. Fixed in two commits after 96214750 (the load crews are never evicted, since the harness reads them on the lobby thread a fast-forward blocks: that is the harness, not a player; the rows grade the sim with N live matches) with a lost-match guard that fails any row without N open crews and N live matches. Any earlier `--report` figure from this harness, including the one behind the provisional `PIRATES_BR_MAX_MATCHES = "2"`, needs a re-measure. Re-run `--scaling 2` at load1 < 6 before stamping a row here. Until it is green, workers stay off on performance-1x (auto = 0) and O6 is not worth a yes.
