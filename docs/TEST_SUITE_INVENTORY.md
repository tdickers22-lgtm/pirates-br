...
| `test-audio-manifest` | public/assets/audio re-read and re-measured with ffmpeg (EBU R128; sub-400 ms clips tiled): every manifest key has >= 1 file and frequent keys >= 3 variants; every file exists, is MP3, matches its byte count, has a LICENSES.md row with an allowed licence (CC0/PD, CC-BY only with a credits.json row); tiers boot <= 1.5 / match <= 3.5 / zones <= 2.0 / music <= 1.0 (reserved, b3.5g) / total <= 8 MB; no orphan files; loudness within 1 LU of -18 one-shot / -26 bed / -24 UI, sample peak <= -0.5 dBFS; one-shots <= 3 s, loops 8-12 s with loop points inside; setWaveBed / setHullCreakIntensity / setWindIntensity one call site each; hullCreakStrain exactly 0 ashore and beyond 15 m | audio-01 / audio-14 / audio-03 / vm:audio:5 (b2.4a); red on 7ddeec86 (HEAD before b2.4a): no manifest + 3 double-writer call sites + no creak model (5 FAIL); first build: wood.plank.3 -19.4 LUFS and sword.clash.1 peak +0.4 dBFS caught (build now converges or fails) |
| `test-audio-models` | pure, fake Web Audio graph: VoiceAllocator never exceeds 64/40/24 under a seeded 200-event burst, every steal takes the lowest priority x gain (oldest on a tie) and a newcomer below every live voice is refused, a high->low tier drop sheds lowest-first; AudioCore ui level reaches master -> glue -> limiter -> destination without the world filter while sfx/ambience/music pass it, combat duck -6/-12 dB within 40 m; SoundEngine.playUiClick (procedural AND sample) lands on the ui path, a world one-shot still passes the world filter, setBusVolume NaN-safe; SampleBank: missing manifest / missing key / 404 / late decode return null without throwing, <= 3 decodes in flight ordered urgent > boot > match > zones, decoded LRU stays under the 40 MB phone cap and evicts, 5 s one-shot and 14 s loop refused, decodeAudioData promise + callback + dual forms, the shipped manifest fits the caps (boot+match decoded 17 MB < 40 MB); b2.4c Spatial: per-category distance gain monotonic non-increasing, air-absorption cutoff continuous, delayFor(139) vs (141) < 10 ms and delayFor(600) = 1.749 s with no cap, cannon crack crossfade 120-220 m without a step, azimuth + rear shade tell behind from ahead, Doppler within 1% of c/(c-v); engine: AudioListener follows setListenerPose, positioned procedural AND sample one-shots run through a DelayNode (d/343) + ONE world PannerNode (HRTF high desktop, equalpower otherwise), whistle playbackRate rides the fly-by Doppler curve | audio-01 / audio-09 / audio-11 / vm:performance:1 (b2.4b); red on 1344d58a+: modules absent (ERR_MODULE_NOT_FOUND); mutations: ui level -> worldFilter (3 FAIL), allocator steals highest (4 FAIL), 5 concurrent decodes + no eviction (2 FAIL: peak 5, 201 MB held) |
| `test-flood-audio` | pure + fake host + fake Web Audio graph: holeGush 0 for a hole 0.1 m above and at the waterline, strictly increasing with depth 0.02..4 m for sizes 1-3 jet and boil (v = sqrt(2 g h), amplitude ~ sqrt(A) v^1.5 soft-saturated), size 3 > 1.3x size 1, boil quieter and < half the cutoff; 10 breaches -> the 6 fastest kept; FloodAudio: 9 breaches -> 6 gush loops each at its breach, no punch on first sight, patch -> plank knock at the breach and the gush stops with a 0.3 s release, a new size-3 breach cracks at the size-3 rate, slosh 0 dry / louder rolling / grows with fill, gurgle 0 <= half full; own hull at 4 m vs the same hull 60 m off: gush 15.2 dB and slosh 15.8 dB apart (>= 12), nothing voiced beyond 70 m; mallet hits == visible hammer blows per hole size (2/3/4, b2-ask-03), each on the swing impact frame (shared repairBlows clock), none before the first swing lands, and the live viewmodel blow position drives it when present; founder cues groan, 3 frame cracks, 2 air releases, suction in order; engine: updateFlood gives each breach its own PannerNode at its x, reaching the output, bucket/reset/NaN frames never throw | audio-02 / vm:audio:1 / vm:audio:2 / vm:liveplay:4 (b2.4d); red on 147759d3+model: engine has no updateFlood (the old path was one centred noise loop); mutations: no hull occlusion (3 FAIL), shallowest kept (4 FAIL), gush above the waterline (3 FAIL), slosh when dry (2 FAIL), hard clamp instead of saturation (2 FAIL) |
| `test-audio-events` | static + fake Web Audio graph + a SampleBank fed from the shipped manifest: every `playSample`/`sampleLayer`/`key:` literal in src/client/audio resolves in manifest.json; every public play*/set*/start*/stop*/update* of SoundEngine has a caller in src/client except an exact owned list (setBusVolume + playUiHover wired by b2.4h; startFire -> b2.3 hook; playGoldCount -> HUD), a wired one fails until its row is deleted; every play* has a route; 25 combat/foley events (cannon, 4 gunshots, sword clash, blade draw, ball/firebomb/bullet impacts, hull hit, ram/rock, big/small splash, keg, chest pickup/stow/open, door open/close, gold, mallet, axe, ui click) each play their own key(s) with <= 3 sample layers and <= 3 procedural sweeteners; with no bank each builds its procedural voice; 12 cannon shots: no back-to-back repeat, 3 variants, pitch jitter <= 1 st with spread > 0.2, level spread 0.3-3.5 dB | audio-01 / audio-09 (b2.4f); red on 08a41354+new keys: 5 FAIL (10 literals, 25 events play no sample, 1 variant 11 repeats, no jitter); mutations: jitter off (2 FAIL), full procedural cannon under the sample (13 primitives) + door keys swapped (1 FAIL), unknown key literal (1 FAIL) |
| `test-ambience` | pure + static, no AudioContext: geyserVoice hiss/roar/cutoff non-decreasing in geyserEruptionLevel at 6 distances and 0 past 170 m; calderaRumble 1 within 30 m, 0 past 260 m; lavaBubble 0 past 70 m; waterfallLevel 0 at 130 m; jungle birds by day inland on lush, insects + frogs at night, 0 at sea, storm hushes; planner: one steam burst per eruption onset, 5-40 birds/min by day, frogs by night, lava bubbles at the rim, nothing at sea; throwing provider -> []; audio settings parse(serialise(s)) = s for 200 random, poison clamps, legacy {volume, muted} loads, apply drives master/mute/4 buses/focus mute/mix; static: VolcanicFx installs the provider, SoundEngine plans zones in setAmbience + bed.waterfall + focus mute + zones preload, MenuController mixer rows | audio-07 / audio-11 (b2.4h); red on cc4bd4f3+Ambience.ts: 3 static FAIL (no provider, no zone planning/waterfall sample/focus mute, no mixer) and 1 planner FAIL that was the gate's own bug (crater counted on the lush zone), fixed before engine work |
...
| `probes/throttled-load-probe` (server tier: builds, own 8091) | built client on an emulated iPhone 14 at 9 Mbit/s, 70 ms RTT, CPU 4x, cold cache: A menu Play clickable <= 3.5 s from navigation; B <= 3 long tasks > 50 ms outside render() in 20 s after the horn; `--mutate=bloat` fails A, `--mutate=busy` fails B | b1.5d, registered at the b1 gate |
| `probes/webkit-ktx2-probe` (browser, no port: page.route) | Playwright WebKit headless (plus SwiftShader Chromium with `--chromium`): a packed KTX2 GLB (default capstan) decodes through GLTFLoader + meshopt + KTX2Loader(/basis/), every map a CompressedTexture with mips, >= 5% of the frame textured, and the unlit frame matches the same model drawn from its JPEG source by the same engine (mean colour <= 8, per-pixel RGB <= 24); `--mutate` (empty transcoder wasm) FAILS | performance-02 (b3.1c) |
...
| test-hud-visibility.mjs | quick (logic) | b1.5f: pure hudVisibility/hudMessagePlan against the spec table: 12 canonical contexts, <=12 always-on desktop / <=8 phone, contradiction rows, every UiRefs id has a writer. b2.2h flood rows (holes-12, mechanicshud-06): floodCard splits open leaks below the live surface (or its 0.15 m wash, LOW tier when no depth) / topside, largest size glyph first; trend arrow from floodingRate (+-0.0005 deadband); 'Taking water fast' at >= 50 % unless falling; bilgeGauge only on the own-hull 30 m card rule (swimming back included), with water, not when hidden in Settings, never dead; alarm slot: sinking > fast flood > outside ring / fire; HudController wiring (floodCard, has('bilgeGauge'), shipFloodingRate, settings-bilge-gauge, no 80 m rule); every flood suite tagged quick in suites.mjs and 'quick' in this inventory. RED on 7280da84..dd7d436c HEAD: 16 FAIL. |
...
| triage-beacons.mjs --self-test / --check --batch bN | quick (logic) / batch gate | b2.0a (critique-14, D35, PLAN rule 14): the bN.0 triage report ($D/batches/bN/triage.json, written by `--report --batch bN [--fly]`) and its gate. --self-test proves 8 red cases red and 3 green cases green (missing report, report for another batch, never pulled = VACUOUS, a >= 3-session signature with neither, a fixCommit unknown to git, a fixCommit without the [bN.0->lane] tag, a prepended slice of another batch or not a z0) plus resolutions surviving a re-pull and the fly-log summary; --check grades the real report. Mutation (drop the VACUOUS rule; accept an unresolved signature) -> 1 FAIL each. |
| test-sailing-handling.mjs | quick (logic) | b2.1a (physics-03/05, D16, PLAN 3.7): section 2 = per-class polars from src/shared/sailing.ts vs the D16 table (top speed sloop 14.0 beam / brig 13.5 broad reach / galleon 13.0 downwind, within 10%, each at its own best point of sail; ordering sloop > brig > galleon at 45 and 90 deg, galleon > brig > sloop at 180; irons <= 1.5; nothing > 14; sloop usable from 40 deg) plus the REAL PhysicsSystem sailing at the polar (steady speed / wind strength within 10% at 5 class/heading rows); section 5 = idealBrace monotonic over 40-180 deg off the wind, <= 65 deg, 0 run / 20-30 broad / 40-50 beam / cap close-hauled, old sign kept; source identity: PhysicsSystem, BotPirate, Match first-sail assist, HudController, ShipRenderer import it and carry no `sin(x) * MAX_SAIL_ANGLE * 0.9x` copy, the renderer draws the brace 1:1, SHIP.MAX_SAIL_ANGLE and SHIP_STATS.maxSpeed mirror sailing.ts; hull params (mass 1:1.9:3.6, drag balancing drive at top speed, design t90 / coast-down in the D16 windows, keel >= 25x, yaw inertia m(L^2+B^2)/12). RED on HEAD 2026-09-24: 21 failures (sim 9.04 vs polar 14.0 sloop beam, MAX_SAIL_ANGLE 86 deg, maxSpeed 15.5/14/11.5, 5 private formula copies, x0.6 renderer). Sections 1 and 3 (real-sim t90/coast/tack, turning) arrive with b2.1b/b2.1d. |
| test-anchor-turn.mjs | quick (logic) | b2.1c (D22, mechanicshud-09 / physics-01): the anchor on the REAL PhysicsSystem at the 0.016 s tick. Pay-out: 2.0 s with no deceleration vs a no-anchor twin (sloop, galleon); bite: peak -d|v|/dt <= 0.6 g and < 0.5 m/s within 1.5-3.0 s of the bite from each class top speed with canvas set, no bow breach; anchor turn: sloop at 12 m/s full rudder turns >= 90 deg in 6 s and ends < 0.5 m/s, rudder centred < 20 deg; helm over at the bite swings every class 45-90 deg (galleon least, mirror-exact for the other helm), swing-weighted pivot point in the forward 20% of the hull, bow travel < half a free twin's; held: canvas set on a held anchor stays < 0.5 m/s, a hull spawned anchored is held from the first tick; shared/anchor.ts state machine (raised/dropping/biting/held, stagger > 5 m/s, raise 3.2 s solo / 2.0 s with two); source identity (PhysicsSystem steps it, no ANCHOR_BRAKE, Match capstan uses anchorRaiseSeconds(hands)). RED on HEAD f1095c17 2026-09-24: 13 failures (peak decel 12.5-14 g, the stop 1.7 s BEFORE the bite, anchor turn 13 deg / 7.3 deg after the bite, galleon 3.3 deg). |
| test-ballistics.mjs | quick (logic) | b2.1f (D17, physics-06 / physics-07): one shared model (src/shared/ballistics.ts, g 9.81, no drag, no kick, muzzle 56 m/s). Range table: the REAL WeaponSystem launch + REAL PhysicsSystem step within 3% of the analytic D17 range at -5.7..35.5 deg, max range 300-312 m (306.0); launch leaves along the barrel at rest; a 150 m broadside from a sloop at 14 m/s stays within 1 m of the inherited-velocity arc (lands >= 30 m ahead); omega x r matches the muzzle's finite difference, heave/roll/pitch rates lift it, PhysicsSystem records heaveRate == d y/dt; server step vs client closed form within 1 cm at 3 s + Game/BotPirate source identity; bot solver with no noise lands within 2 m of a moving sloop at 150 m >= 95% (120/120); grazing skip: 3 deg at 55 m/s skips 1-2 times and flies >= 20 m past first contact, 20 deg goes in. RED on HEAD d04515aa 2026-09-24: 21 failures (range 28-92% long, max 391.8 m, vy +5 at 0 pitch, broadside 45.1 m off, 12.9 m server/client split at 3 s, bot 0/120, no skip). |
| test-seakeeping.mjs | logic (~5 s) | b2.1e (physics-09/10, liveplay-02, PLAN 3.7 sea-keeping): the REAL PhysicsSystem at the 0.016 s tick measured against an INDEPENDENT 13 x 7 least-squares wave plane over the footprint. Calm, sails struck: roll/pitch RMS <= 0.5 deg, heave RMS <= 0.08 m per class; free-decay roll after applyHullImpulse: period sloop 3-4 / brig 4.5-5.5 / galleon 6-8 s, damping ratio 0.2-0.4 (log decrement); beam-reach heel sloop 7-9 / brig 5-7 / galleon 3-5 deg to leeward, mirror on both tacks, < 0.3 deg luffing or struck; turn heel carries omega's sign (outward) with steady <= 3 deg; a hull anchored in her Match berth draws (position.y + heave) within +-0.35 m of its mean in weather 0 while the open-sea twin at the same point moves > 0.35 m; applyPointLoad sinks and lists/trims toward the load; storm: finite, clamped, slam/green-water events for a sloop, none in calm; >= 21 stations. RED on HEAD 652b328c 2026-09-24: 36 failures (roll RMS 2.07/1.99/1.86 deg, pitch 1.62/1.36/1.05, heave 0.089/0.107/0.139 m; decay period 4.21/4.15/4.02 s all classes, zeta < 0; heel 2.54/1.75/2.23 deg with 0.5-0.7 deg luffing/struck; berth 1.47 m; no applyPointLoad, no events, no stations). |
| test-ship-collision-mass.mjs | quick (logic) | b2.1g (physics-08, PLAN 3.7 collisions): rams on the REAL PhysicsSystem at the 0.016 s tick after the truce, canvas struck, bow to bow. Galleon ramming a stopped sloop at 8 m/s keeps >= 70% of her way vs a twin that hit nothing (0.4 s after contact); the sloop ramming the galleon keeps <= 30%; sloop:galleon ram damage = mass ratio 3.6 +-20% in both rams and the sloop loses more planks; hullJolts (each hull's own delta-v, for the crew stagger) sloop:galleon ~3.6, sloop > 5 m/s, galleon < 2.5; sloop vs sloop shares the damage and keeps 30-50%; three pairings bit-equal with the ships array reversed; a sea rock is infinite mass (head-on rebound e 0.15-0.25, jolt (1+e) v). RED on HEAD 0cc67f12 2026-09-24: 12 failures (galleon keeps 48.4%, sloop keeps 49.6%, damage ratio 1.00 both ways, holes 2/2, no hullJolts, all three array-order rows differ, rock e 0.448). |
| test-flooding.mjs | quick (logic, ~4 s) | the flooding integration suite on the REAL Match/PhysicsSystem/FloodSystem: section-7 untended windows (sloop 55-80 s, galleon 82-115 s), sqrt law through FloodSystem, heeled low-side rate, saturation block, hole sizes 1-3 + enlarge within 0.6 m + repair 1.6/2.4/3.2 s (b2.2b), SoT bucket rules (b2.2d: deck scoop refused, inboard heave returns in 1.2 s, outboard heave removes, bots walk the cycle, no auto-carpenter), aimed plank repair + progress kept 3 s (b2.2e), founder profile settle/trim/plunge + stage events once (b2.2g). Quick despite ~4 s: every flood gate runs pre-commit (b2.2h, holes-12). |
| test-flood-model.mjs | quick (logic, ~1.5 s) | b2.2a (holes-01/02/10, D15, PLAN 3.6): the shared flood model in src/shared/flooding/ on the REAL updateShipFlooding at 1/60 s with the settle in the loop. Section-7 windows kept (two untended waterline holes: sloop 55-80 s, galleon 82-115 s, brig between); Q(1 m)/Q(0.2 m) 1.9-2.3; a hole 0.3 m above calm water stays dry; the inside head cuts Q once the hole is under the hold water; per-class loft fill tables monotone, fill 0 = sole, fill 1 = deck, fill 0.5 = an independent loft half-volume integration; stock hulls never pump dry, reinforced drains at 0.25x BAIL_RATE; design race (b2.2a2): BAIL_RATE beats a small sloop hole 0.2 m under, the scoop/heave cycle delivers BAIL_RATE within 5%, one bailer holds one small hole and still loses to three small (sloop), bailer + pump hold three small on every class. Prints the per-class/size time-to-founder table (held rows exit at equilibrium). RED on HEAD src: module absent; RED on BAIL_RATE 0.014: 0.2 m race. |
| test-flood-trim.mjs | quick (logic, ~0.4 s) | b2.2c (holes-02, liveplay-02, PLAN 3.6 water mass): the REAL PhysicsSystem at 0.016 s, sloop in open water, graded only on ship.roll / position.y. Floods to 0.8 through two starboard (-x) breaches: roll >= 4 deg starboard-low; every hole planked + one bailer: the list (5.5 s mean roll) stays >= 3 deg until the water is under 0.2; mean draft +>= 0.5 m from fill 0 to 0.8; the same beam sea at fill 0.5 rolls >= 1.2x the dry hull (prints the slower period); a 0.1 rad blow at fill 0.5 is within 0.01 rad of an untouched twin (separate world, same sea) 6-10 s later. RED on HEAD src: list after patching -2.61 deg (snaps level and overshoots), beam-sea ratio 1.00. RED on SLOSH_ZETA 0: settle 0.0107 rad. |
| test-hold-wading.mjs | quick (logic, ~2.4 s) | b2.2f (holes-07): the REAL Match.applyInput + PhysicsSystem at 62.5 Hz, a pirate in his own hold walking aft of the companionway, graded on measured deck-relative speed, swimTimer, health, state. Water 0.2: speed within 2% of dry, swimTimer 0; water 0.5: <= 0.6x dry on every class (sloop/brig wade 0.55x, galleon already afloat); water 0.95: buoyed off the sole, still aboard and 'alive', drowning clock running, swim <= 0.6x dry, canScoop true; a full sloop hold (head under the beams) runs the clock 4x and drowns him; hold surface tilts world-level with roll/pitch and clamps to sole/deck; sampleHoldWater on a JSON copy of the wire fields is bit-identical (client predictor). RED on HEAD src (hook stashed): 12 FAIL. RED on holdSpeedCap wade -> 99: both wade rows. |
| test-island-wading.mjs | quick (logic, ~1 s) | b2-ask-01: the REAL stepPirate + PhysicsSystem at 62.5 Hz, a pirate walking along the shoreline in 0.25..0.7 m of water on up to 12 map walks; median horizontal speed = 0.55x MOVE_SPEED +-10%, no walk above the cap, crouch-wade = 0.55 x 0.55 x walk +-15%. RED on HEAD (per-tick multiply): median 0.16 m/s (0.03x walk). |
| test-hold-water-geometry.mjs | quick (logic, ~0.3 s) | b2.3a (holes-01 / liveplay-01): the real client module ship/holdWater.ts. 200 seeded random (fill, roll, pitch, slosh) per class: every point the clip calls water lies inside the TRUE loft (hullSurfacePointAt) and the sole/deck band; a level hold carries water over >= 90% of the centreline at 0.1..0.9; surface == shared holdWaterSurfaceLocalY; fill 0.5 == fill table; full EXACTLY at 1.0, 0.95 still under the deck; roll 0.2 port vs starboard >= 0.3 m, signs; slosh re-sim reverses and decays, still hull adds nothing; no literal 0.8 in the water code; grids 24x12/12x6/6x4. RED on HEAD: module absent. RED on HOLD_WATER_INSET -> -0.1: 3 FAIL (overshoot 0.109-0.118 m). |
| test-water-environment.mjs | quick (logic, ~0.3 s) | b2.3f (holes-07): the real client module rendering/waterEnvironment.ts on the real hold-water clip/plane. Eye under the hold surface inside the hull footprint -> depth > 0 (== surface - eye), every class, level/rolled/trimmed; beyond the lining, beyond the hold's length, under the keel, above the surface, dry -> 0. combineWaterDepth = max(sea, hold), hold palette only when the hold is deeper, smooth blend. 95% FogExp2 fog inside 4-6 m at every fill/agitation, darker at night. Hatch caustic flicker moves by day under the hatch, 0 at 6 m and at night. Game.updateWaterEnvironment source pins (muffle = combined depth, ocean underside = outside depth, applyHoldUnderwater, getCameraWaterState). No material/fog/shader constructed (0 new programs). RED on HEAD: module absent, 6 Game wiring FAILs. RED on the footprint/sole checks removed: 22 FAIL. Browser side: hold-water-probe under-hold rows. |
| test-flood-fx-model.mjs | quick (logic, ~0.3 s) | b2.3b (holes-08): the real client module ship/floodFx.ts. Torricelli exit speed 0.2 m -> 1.98 +- 0.05, 1.0 m -> 4.43 +- 0.05, 0 when dry, capped at MAX_HEAD; the client flooding predicate and q equal the server's evaluateHoleFlood on 1,000 seeded random hole/pose/wave/storm samples (both outcomes and the 0.2-0.3 m band exercised; prints how often the retired +0.2 m client margin disagreed, 45/1000); a hole under the hold water boils (not jets), the same hole with a dry hold jets at sqrt(2 g h), a hole 0.5 m above the sea does nothing; ballistic time of flight and far-side reach cap; 300 ms release after a patch; jets through the first 60% of the founder. RED on HEAD: module absent. RED on the legacy predicate (flooding = depth >= -0.2): 1 FAIL (client != server). |
| probes/hold-water-probe.mjs | gate (browser, heavy, ~3 min SwiftShader, own 3101/8091 stack) | b2.3a: pins the drawn fill/attitude via ShipRenderer.setHoldWaterDebug (freezes the drawn hull), standard hold camera (0.3 m under the deck beams, 30% aft, 0.65 rad down), fixed teal classifier on the lower half: water >= 15/35/55% at 0.25/0.5/0.75; mean luminance >= 0.12 noon / 0.06 night at 0.5; roll 0.2 port vs starboard waterline >= 0.3 m read from the live renderer plane. RED on the first camera pose (eye heaved out through the deck): f50/f75 0% and noon luminance 0.102. |

### b2.1h additions (2026-09-24)

- `test-tick-budget` — per-match server tick cost from the Match's own `TickProfiler` (lap timers inputs/bots/combat/physics/wildlife/storm/rest/snapshot, 2048-tick ring): solo, 12 bot hulls, seed 20260801, bot early peace off, 60 s fast-forward, storm forced to phase 5, 30 s graded: p99 <= 4.0 ms, p50 <= 2.0 ms; the 1.5x noise allowance is printed, never applied. Built-in control: a 3 ms busy loop in `updateWildlife` must break both limits; `MUTATE=wildlife3ms` runs the graded window mutated. Logic tier, ~30 s, not quick. RED at landing (p50 2.21 / p99 4.43 ms on the Air): the remaining cost is wildlife walker collision and ship-island pushout, see the b2.1 lane report.

### b2.3d additions (2026-09-25)

- `test-ship-hole-vis` section 6 (quick, ships-06 / vm:assets:2) — breach geometry v2 from `src/client/rendering/ship/breach.ts`: torn-outline radius CV >= 0.15 over 60 ids (jag alone, before the stretch), stretched along the strake (along/across mean >= 1.3, each >= 1.1), the GLSL twin parsed and evaluated against the JS outline (|diff| < 1e-5), 3 sizes drive the discard radius, deterministic by hole id across two ShipRenderer builds, 0.08 m rim wall (>= 0.06), 5-9 plank ends bent 20-50 deg inboard, splinters inboard, drawn edge == shader cut (0.95 r cut / 1.05 r kept), 1 draw per torn-edge family, key scheme `hull-hole-discard-8|...` kept, patch = 2 (size 1) or 3 planks within 5 deg of the strake tangent measured independently from `hullSurfacePointAt`. RED: HEAD has no breach.ts; mutation (harmonics 0.16/0.13 -> 0.05/0.04, no teeth) fails the CV row (0.101); the first build projected the tangent onto the section normal and failed the patch rows at 6.0 / 7.6 deg.
| test-tool-assets.mjs | quick | b2.3h flood-loop tool GLBs (tool_bucket, tool_planks, tool_hammer): LOD0 3-10k / LOD1 1.5-3k / LOD2 <=700 tris, nodes bucket-water/hammer-head/hammer-haft (+_lod1/_lod2), hammer grip at origin head +Y face -Z, bucket bottom -0.10 rim +Y water inside, one atlas material, factories clone them. Red: `--models <empty dir>` (3 FAIL) |
| test-ship-hold-cull.mjs | quick | b2-device-04: the hold interior v2 is culled on a SEALED hull beyond 60 m inside the detail band (all 3 classes, high + low): galleon 108.7k -> 93.6k verts (-15.1k), brigantine -10.6k, sloop -7.3k; only meshes whose material nothing outside the hold uses are hidden (no split draw, no lost deck timber, no light toggled); an open breach, a foundering hull, the local crew's own hull and < 60 m draw all of it, and a patch re-culls. Red on HEAD: 6 FAIL (0 verts saved); mutations (drop the open-breach term, drop the own-hull term) each fail 6 rows. |

### b3.1a additions (2026-09-25)

| `test-model-transport` | TRANSPORT: every GLB the client fetches is `public/assets/models/packed/<name>.glb` from `scripts/pack-models.mjs`, packed from today's source bytes (srcSha256), EXT_meshopt_compression required, KHR_mesh_quantization wherever a NORMAL exists; decoded it equals the source (node names/TRS, material names, primitive modes, tris per primitive, POSITION float32 outside QUANTISED_POSITION_FAMILIES, bounds within 2^-15 of extent); brotli per AssetLibrary set boot <= 0.6 / world <= 6 / lazy <= 12 / far <= 1 MB; client fetches via modelManifest.modelUrl with the meshopt decoder wired | yes | b3.1a; logic tier (~1.9 s), tsx; **`--mutate`** ships one raw GLB and must FAIL; `--consumers` also runs test-asset-merge, test-far-lod-integrity, test-asset-bounds, test-hero-assets on the packed bytes (red while test-asset-bounds is red on the raw tool_hammer, see b3.4g) |
| `test-texture-budget` | TEXTURES: no image/png or image/jpeg inside a packed GLB outside `pack-models.mjs` TEXTURE_ALLOWLIST (empty); every texture KHR_texture_basisu with a full mip chain, ETC1S (BasisLZ) on baseColor/emissive/ORM, UASTC+zstd on normals; GPU MB per tier (high/balanced/low/phone/iPad) after AssetLibrary TEXTURE_TOP_MIP_CAP (512 family / 1024 hero viewmodel on capped tiers) <= 0.5 x the D27 residency column; ETC1S top level transcoded by three's basis_transcoder vs the Blender source JPEG: mean CIE76 deltaE < 3; public/basis byte-identical to three's transcoder with .br/.gz siblings | yes | b3.1c; logic tier (~3 s); **`--mutate`** leaves one JPEG and must FAIL |
