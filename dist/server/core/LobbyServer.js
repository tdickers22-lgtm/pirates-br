...
const MATCH_GC_AFTER_END_MS = 60000;
const ENDED_MATCH_DETACH_MS = 25000; // auto-return-to-menu after this if client doesn't act
...
export class LobbyServer {
    constructor() {
        ...
        this.clientToMatch = new Map(); // clientId → matchId
    ...
    onDisconnect(session) {
        ...
        if ((session.state === 'in_match' || session.state === 'match_ended') && session.matchId && session.matchPlayerId) {
            ...
            if (match)
                match.removeClient(session.matchPlayerId, false);
    ...
    routeMessage(session, msg) {
        ...
        if ((session.state === 'in_match' || session.state === 'match_ended') && session.matchId && session.matchPlayerId) {
            const match = this.matches.get(session.matchId);
            if (match)
                match.handleClientMessage(session.matchPlayerId, msg);
        }
    }
    // ─── Lobby handlers ──────────────────────────────────────────
    handleSetName(session, msg) {
        const payload = (msg.payload ?? {});
        const name = (payload.name ?? '').trim().slice(0, 24);
        if (!name)
            return this.lobbyError(session, 'Name cannot be empty');
        session.name = name;
        const stats = this.stats.ensure(name);
        this.sendStats(session, stats);
    }
    handleCreateParty(session, _msg) {
        if (!session.name)
            return this.lobbyError(session, 'Set a name first');
        ...
        if (this.parties.has(code))
            return this.lobbyError(session, 'Could not create party — try again');
        const party = {
            code,
            hostId: session.id,
            members: [session.id],
    ...
    handleJoinParty(session, msg) {
        ...
        if (!party)
            return this.lobbyError(session, `Party ${code || '????'} not found`);
        ...
        if (party.members.length >= PARTY_CAPACITY)
            return this.lobbyError(session, 'Party is full');
        if (session.state === 'in_match' || session.state === 'match_ended') {
            return this.lobbyError(session, 'Already in a match');
        }
        if (session.state === 'queue')
            this.removeFromQueue(session);
        if (session.state === 'party' && session.partyCode !== code)
            this.removeFromParty(session, true);
    ...
    handleLeaveParty(session) {
        if (session.state !== 'party')
            return;
        this.removeFromParty(session, true);
    }
    handleUpdatePartySettings(session, msg) {
        if (session.state !== 'party' || !session.partyCode)
            return;
        const party = this.parties.get(session.partyCode);
        if (!party || party.hostId !== session.id)
            return this.lobbyError(session, 'Only the host can change settings');
        const payload = (msg.payload ?? {});
    ...
    handleStartMatch(session) {
        ...
        if (party.hostId !== session.id)
            return this.lobbyError(session, 'Only the host can start');
    ...
    handleQueueLeave(session) {
        if (session.state !== 'queue')
            return;
        this.removeFromQueue(session);
    }
    handleSoloStart(session, msg) {
        if (!session.name)
            return this.lobbyError(session, 'Set a name first');
        if (session.state === 'in_match' || session.state === 'match_ended') {
            return this.lobbyError(session, 'Already in a match');
    ...
    handleReturnToMenu(session) {
        if (session.state !== 'in_match' && session.state !== 'match_ended')
            return;
        if (session.matchId && session.matchPlayerId) {
            const match = this.matches.get(session.matchId);
            if (match)
                match.detachClient(session.matchPlayerId);
        }
        this.clientToMatch.delete(session.id);
        session.state = 'menu';
        ...
        if (session.name)
            this.sendStats(session, this.stats.ensure(session.name));
    }
    /**
    ...
    maybeClearPartyInMatch(code) {
        if (!code)
            return;
        const party = this.parties.get(code);
        if (!party || !party.inMatch)
            return;
    ...
    // ─── Party helpers ───────────────────────────────────────────
    removeFromParty(session, notifyRest) {
        const code = session.partyCode;
        if (!code)
            return;
        session.partyCode = undefined;
    ...
    broadcastLobby(party) {
        ...
        for (const id of party.members) {
            const c = this.clients.get(id);
            if (c)
                this.send(c.ws, { type: 'lobby_update', ts: Date.now(), payload });
        }
    }
    // ─── Queue helpers ───────────────────────────────────────────
    ...
    broadcastQueue(starting = false) {
        ...
        for (const id of this.queue) {
            const c = this.clients.get(id);
            if (c)
                this.send(c.ws, { type: 'queue_update', ts: Date.now(), payload });
        }
    }
    tryDispatchQueue() {
        if (this.queue.length === 0)
        ...
        if (!fastReady && !slowReady && !enoughHumans)
            return;
    ...
    placeClientIntoMatch(session, match, source, partyCode = null) {
    ...
    findClientByPlayerId(playerId) {
        for (const session of this.clients.values()) {
            if (session.matchPlayerId === playerId)
                return session;
        }
        return null;
    }
