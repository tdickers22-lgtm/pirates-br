...
export class WeaponSystem {
    ...
    tryFire(player, ship, yaw, pitch, cannonIndex, options) {
        if (player.state === 'eliminated')
            return [];
    ...
    tryMeleeAttack(attacker, targets, yaw, options) {
        ...
        for (const target of targets) {
            if (target.id === attacker.id || target.state === 'eliminated')
                continue;
    ...
    fireShipCannon(player, ship, yaw, pitch, cannonIndex) {
        if (cannonIndex >= ship.cannonCooldowns.length)
            return [];
        if (ship.cannonCooldowns[cannonIndex] > 0)
            return [];
        ...
        if (!usesSuperShot && (!consumesInventory || usedIdx < 0))
            return [];
        if (usesSuperShot) {
            ...
            if (ship.inventory[usedIdx].qty <= 0)
                ship.inventory.splice(usedIdx, 1);
        }
    ...
    startReload(player) {
        ...
        if (!weapon || weapon.reloading)
            return;
        const def = WEAPONS[weapon.weaponId];
        if (def.melee || weapon.ammo >= def.ammoMax)
            return;
        weapon.reloading = true;
        weapon.reloadTimer = def.reloadTime;
    }
    tickCannons(dt, ships) {
    ...
    createDefaultWeapons() {
        return [
            { weaponId: 'blunderbuss', ammo: 1, reserve: 5, reloading: false, reloadTimer: 0 },
            { weaponId: 'eye_of_reach', ammo: 1, reserve: 5, reloading: false, reloadTimer: 0 },
            { weaponId: 'flintknock', ammo: 1, reserve: 5, reloading: false, reloadTimer: 0 },
            { weaponId: 'cutlass', ammo: 0, reserve: 0, reloading: false, reloadTimer: 0 },
        ];
    }
    getSpreadMultiplier(player, weaponId, aiming) {
    ...
    applyConeSpread(baseDirection, spreadRad) {
    ...
    normalizeVector(vector, fallback) {
    ...
    getConstrainedCannonAim(ship, cannonIndex, yaw, pitch) {
        const stats = SHIP_STATS[ship.type];
        const cannonsPerSide = Math.max(1, stats.cannonCount / 2);
