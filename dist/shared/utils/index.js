...
export function buildSeaRockColliders(radius, height, rotation, variant) {
    const colliders = [];
    ...
    const boundsRadius = colliders.reduce((max, collider) => (Math.max(max, Math.hypot(collider.localX, collider.localZ) + collider.radius)), radius);
    return { boundsRadius, colliders };
}
export function getSeaRockColliders(rock) {
    if (rock.colliders?.length)
        return rock.colliders;
    return buildSeaRockColliders(rock.radius, rock.height, rock.rotation, rock.variant).colliders;
}
export function getSeaRockBoundsRadius(rock) {
    return rock.colliderBoundsRadius || buildSeaRockColliders(rock.radius, rock.height, rock.rotation, rock.variant).boundsRadius;
}
export function seaRockColliderWorldCenter(rock, collider) {
...
export function intersectRaySeaRock(origin, direction, range, rock, padding = 0) {
    ...
    if (broadA > 0.000001) {
        ...
        if (broadDisc < 0 && broadC > 0)
            return null;
    }
    let best = null;
    const testT = (t, cx, cz, radius, minY, maxY) => {
        if (t < 0 || t > range || (best !== null && t >= best))
            return;
        ...
        if (Math.hypot(x - cx, z - cz) <= radius + 0.0001)
            best = t;
    };
 ...
 */
export function getOceanRoughness(t) {
...
export function getIslandSurfaceY(island, x, z) {
    ...
    const primaryHill = hillContribution(profile.primaryHillAngle, profile.primaryHillOffset, 0.34 + profile.mesaBias * 0.08 + peakBoost * 0.04, (0.018 + profile.heightProfile * 0.022) * (1 + peakBoost * mountainBoostFactor) * (isTwin || isArchipelago ? 1.4 : isMountain ? 1.6 : 1));
    const secondaryHill = hillContribution(profile.secondaryHillAngle, profile.secondaryHillOffset, 0.3 + profile.secondaryHillScale * 0.1, (0.01 + profile.secondaryHillScale * 0.018) * secondaryAmp);
    const tertiaryHill = profile.tertiaryHillScale > 0
        ? hillContribution(profile.tertiaryHillAngle, profile.tertiaryHillOffset, 0.26 + profile.tertiaryHillScale * 0.1, (0.006 + profile.tertiaryHillScale * 0.014) * tertiaryAmp)
        : 0;
...
export function getShipCompanionwayConfig(stats) {
