import type { Player, Ship, Island, StormState } from '../../shared/types/index.js';
...
export class BotSystem {
  ...
  private trimSails(ship: Ship, t: number) {
  ...
  private calculateCannonPitch(ship: Ship, target: Ship): number {
    const dx = target.position.x - ship.position.x;
    const dz = target.position.z - ship.position.z;
    const d = Math.sqrt(dx * dx + dz * dz);
