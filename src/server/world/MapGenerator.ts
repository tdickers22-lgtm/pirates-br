...
import type { Island, IslandBarrel, IslandCave, IslandDock, IslandNpc, IslandProfile, IslandTavern, ItemType, TreasureChest, UpgradeStation, ShipUpgradeType, Vec3, Ship, WildlifeAnimal, WildlifeType, SeaRock } from '../../shared/types/index.js';
...
export class MapGenerator {
  ...
  private generateIslandProfile(radius: number, islandIndex: number): IslandProfile {
    ...
    const styles: IslandProfile['terrainStyle'][] = [
      'tropical', 'tropical',
    ...
    if (radius > 96) {
      const largeStyles: IslandProfile['terrainStyle'][] = ['mountain', 'plateau', 'twin'];
      terrainStyle = largeStyles[(islandIndex + Math.floor(Math.random() * largeStyles.length)) % largeStyles.length];
    } else if (radius < 46) {
      const smallStyles: IslandProfile['terrainStyle'][] = ['tropical', 'rocky', 'archipelago'];
  ...
  private generateCaves(island: Island, islandIndex: number): IslandCave[] {
    const style = island.profile.terrainStyle;
    let count = 0;
    if (style === 'rocky') count = 1 + (Math.random() < 0.6 ? 1 : 0);
    else if (style === 'mountain') count = Math.random() < 0.7 ? 1 : 0;
    else if (style === 'plateau') count = Math.random() < 0.4 ? 1 : 0;
    else count = Math.random() < 0.25 ? 1 : 0;
    if (count === 0) return [];
  ...
  private generateTavern(island: Island, islandIndex: number): IslandTavern | null {
    // Only the bigger islands with docks host taverns, ~3 across the map.
    if (!island.dock) return null;
    if (island.radius < 58) return null;
  ...
  private generateDock(island: Island): IslandDock | null {
    const dockChance = island.radius > 70 ? 0.82 : island.radius > 55 ? 0.62 : 0.36;
    if (Math.random() > dockChance) return null;
    ...
    const moorSide = (Math.random() < 0.5 ? -1 : 1) as -1 | 1;
  ...
  private generateChests(island: Island): TreasureChest[] {
  ...
  private generateBarrels(island: Island): IslandBarrel[] {
  ...
  private generateStoryNpcs(island: Island, islandIndex: number): IslandNpc[] {
  ...
  generateSeaRocks(
    ...
    for (let attempt = 0; attempt < attempts && rocks.length < SEA_ROCKS.COUNT; attempt++) {
      ...
      const variant = randInt(0, 2) as 0 | 1 | 2;
  ...
  generateShipSpawns(islands: Island[]): Array<{ position: Vec3; rotation: number; type: (typeof SHIP_TYPES)[number] }> {
    ...
    for (let i = 0; i < WORLD.SHIP_COUNT; i++) {
      ...
      if (!pos) pos = { x: randRange(-700, 700), y: 0, z: randRange(-700, 700) };
