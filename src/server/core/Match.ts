...
export class Match {
  ...
  constructor(opts: MatchOptions) {
    ...
    this.fauna = new FaunaSystem(this.rng, {
      ...
      absorbWithArmor: (target, amount) => this.absorbWithArmor(target, amount),
      noteDamageSource: (playerId, source) => this.noteDamageSource(playerId, source),
      noteEnvironmentalDamage: (player, source, amount, origin) => this.noteEnvironmentalDamage(player, source, amount, origin),
    });
  ...
  // ── Fauna ─────────────────────────────────────────────────────────────────
  // The bodies moved to server/systems/FaunaSystem.ts (WILD-01 a, split-as-you-
  // touch). These stay as one-line delegates so the tick order above is still
  // read in one place and the existing suites (test-shark-lunge) keep driving
  // the match object rather than reaching into a system they never knew about.
  private updateSharks(dt: number) {
    this.fauna.updateSharks(dt, this.state);
  }

  private updateWildlife(dt: number) {
    this.fauna.updateWildlife(dt, this.state);
  }
