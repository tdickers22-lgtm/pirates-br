...
export class Game {
  ...
  private syncSharks(dt: number) {
    ...
    for (const shark of sharks) {
      ...
      let created = false;
      if (!mesh) {
        mesh = buildSharkMesh(this.renderer.getQuality());
        this.environment.add(mesh);
      ...
      // Pose and skeleton belong to SharkRenderer: the same attack-state
      // numbers drive the skinned hero's mixer and the far puppet's four
      // pivot nodes, and the mixer's distance LOD lives with them.
      updateSharkPose(
        mesh, dt, attackState, shark.attackTimer ?? 0,
        Math.hypot(shark.velocity.x, shark.velocity.z),
        this.ocean.getTime(),
        this.renderer.camera.position.distanceTo(mesh.position),
      );
    }
  ...
  private syncWildlife(dt: number) {
    ...
    for (const animal of this.state.wildlife ?? []) {
      ...
      let created = false;
      if (!mesh) {
        mesh = buildWildlifeMesh(animal, this.renderer.getQuality());
        this.environment.add(mesh);
