...
export function updateInstanceLod(
  ...
  for (let b = 0; b < batches.length; b++) {
    ...
    if (batch.far && batch.near) {
      ...
      //
      // THIS IS ONLY SOUND BECAUSE THE FAR FILES ARE WATERTIGHT. Until [rocks]
      // (2026-09-09) build_far_lods.py ran Collapse on split vertices and the
      // far rocks were 20% of the triangles with 42% of the surface and 76-93
      // boundary loops each, so this line put a see-through boulder in the
      // player's face on Low. test-far-lod-integrity now grades every far
      // file as a surface. A near BAND was tried instead — draw near while any
      // instance of the batch is within 45 m of the camera — and measured out:
      // for every type it put the whole island back on near (724k/470k
      // inland, 635k/560k cave); for rock batches alone 589k/470k and
      // 601k/560k, because one boulder in reach makes its whole batch near.
      // A batch's count is a scale-sorted prefix, so nothing here can thin
      // the player's own island by distance; near rocks at arm's length on
      // Low need a per-instance near/far split (re-pack + propSlots remap).
