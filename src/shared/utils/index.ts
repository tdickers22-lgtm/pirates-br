import type { Island, IslandDock, Ship, ShipType, Vec3, Vec2 } from '../types/index.js';
...
export function getIslandSurfaceY(island: Island, x: number, z: number): number {
  ...
  const primaryHill = hillContribution(
    ...
    0.34 + profile.mesaBias * 0.08,
    0.018 + profile.heightProfile * 0.022,
  );
  const secondaryHill = hillContribution(
    ...
    0.01 + profile.secondaryHillScale * 0.018,
  );
  const tertiaryHill = profile.tertiaryHillScale > 0
    ? hillContribution(
      ...
      0.006 + profile.tertiaryHillScale * 0.014,
    )
    : 0;
  const angleNoise = (
