...
if (dead) {
  ...
  ok('any hold is brief — the tide converts it inside the grace + a tick',
    !sawHeld || heldSeconds <= 25,
    `held for ${heldSeconds.toFixed(1)}s (grace is 10s)`);
...
writeFileSync(`${OUT}/endgame-live-report.json`, JSON.stringify({
  world, log, results, trace: trace.filter((_, i) => i % 5 === 0), bands, bleedTail: bleed.slice(-40), respawnTrace, errors: errors.slice(0, 20),
}, null, 1));
