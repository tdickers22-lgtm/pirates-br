...
// source public/assets/models/<name>.glb answers with packed/<name>.<hash8>.glb after
...
import { packIO, SRC_DIR, packedFile, readManifest, listSources } from '../pack-models.mjs';

const io = await packIO();
const decoded = new Map();
const manifest = readManifest();
for (const name of listSources()) {
  const file = packedFile(name, manifest); // packed/<name>.<hash8>.glb (b3.1b)
