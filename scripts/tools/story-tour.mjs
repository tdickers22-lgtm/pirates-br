...
// the widow's lantern. node scripts/tools/story-tour.mjs [outDir]
//
// Restored by w0.fixup: the w0.2 sweep deleted scripts/story-tour.mjs, but PLAN
// lane 2.5's gate ("story-tour shots of mermaid_shrine/kraken_wreck
// before/after") and the TEX-01 / TAVERN-01 rows cite it. Doc-cited tooling
// lives in scripts/tools/. Three things changed on the way in: it read the
// HUMAN port (3000) — it now reads PIRATES_BR_URL (default :3101, the test
// stack); it asked for Metal at 1600x900 — it now runs SwiftShader at 960x540
// like every other rig here (ONE GPU browser at a time on this machine); and
// waitForFunction's options bag is its THIRD argument, not the second.
...
await page.goto(`${ROOT_URL}/?debug&quality=high`, { waitUntil: 'domcontentloaded' });
