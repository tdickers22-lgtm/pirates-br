...
Counts (generated by `node scripts/glb-census.mjs --write`): **122 GLBs on disk = 83 in `ASSET_NAMES` + 16 `_far` LODs from `FAR_ASSET_NAMES`, 23 unwired (cannon, cannon_far, capstan, capstan_far, castaway_camp_far, crow_roost_far, dig_site_far, gallows_far, gibbet_cage_far, kraken_wreck_far, mermaid_shrine_far, mine_head_far, parley_table_far, rum_still_far, ship_lantern, ship_lantern_far, skull_totem_far, smuggler_cache_far, whale_skeleton_far, wheel, wheel_far, widow_memorial_far, wrecker_tower_far).** This line is the contract — code comments quoting a different number are stale.
