...
Counts (generated by `node scripts/glb-census.mjs --write`): **106 GLBs on disk = 82 in `ASSET_NAMES` + 16 `_far` LODs from `FAR_ASSET_NAMES`, 8 unwired (cannon, cannon_far, capstan, capstan_far, ship_lantern, ship_lantern_far, wheel, wheel_far).** This line is the contract — code comments quoting a different number are stale.
