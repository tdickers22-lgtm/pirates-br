...
Counts (generated by `node scripts/glb-census.mjs --write`): **78 GLBs on disk = 63 in `ASSET_NAMES` + 15 `_far` LODs from `FAR_ASSET_NAMES`, 0 unwired.** This line is the contract — code comments quoting a different number are stale.
