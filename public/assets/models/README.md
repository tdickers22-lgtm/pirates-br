...
Counts (generated by `node scripts/glb-census.mjs --write`): **127 GLBs on disk = 87 in `ASSET_NAMES` + 20 `_far` LODs from `FAR_ASSET_NAMES`, 20 unwired (castaway_camp_far, crow_roost_far, dig_site_far, gallows_far, gibbet_cage_far, kraken_wreck_far, mermaid_shrine_far, mine_head_far, parley_table_far, pirate_clips, pirate_fp_arms, rum_still_far, skull_totem_far, smuggler_cache_far, tool_bucket, tool_hammer, tool_planks, whale_skeleton_far, widow_memorial_far, wrecker_tower_far).** This line is the contract — code comments quoting a different number are stale.
