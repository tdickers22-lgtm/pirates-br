...
Counts (generated by `node scripts/glb-census.mjs --write`): **125 GLBs on disk = 83 in `ASSET_NAMES` + 16 `_far` LODs from `FAR_ASSET_NAMES`, 26 unwired (cannon, cannon_far, capstan, capstan_far, castaway_camp_far, crow_roost_far, dig_site_far, gallows_far, gibbet_cage_far, kraken_wreck_far, mermaid_shrine_far, mine_head_far, parley_table_far, rum_still_far, ship_lantern, ship_lantern_far, skull_totem_far, smuggler_cache_far, tool_bucket, tool_hammer, tool_planks, whale_skeleton_far, wheel, wheel_far, widow_memorial_far, wrecker_tower_far).** This line is the contract — code comments quoting a different number are stale.
...
| rowboat.glb | Weathered clinker tender: lapstrake planking, sawn frames, thwarts, sprung plank, shipped + snapped oars, painter coil, tribute offerings in the bilge | 1.12×0.98×2.65, 3.4k tris — wired (`ASSET_NAMES` + `PROP_COLLIDERS`); no scatter pass yet |
| signal_pyre.glb | Unlit beacon: crib-stacked logs, brush bundles, open tar barrel w/ iron hoops + tar runs, pitch bucket, discarded lid | 2.44×2.13×1.60, 2.5k tris — wired (`ASSET_NAMES` + `PROP_COLLIDERS`); no scatter pass yet |
